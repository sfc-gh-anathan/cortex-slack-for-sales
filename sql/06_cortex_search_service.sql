...
select relative_path, SNOWFLAKE.CORTEX.PARSE_DOCUMENT(@SLACK_SALES_DEMO.SLACK_SCHEMA.SLACK_PDFS,relative_path,{'mode':'LAYOUT'}) as data
from directory(@SLACK_SALES_DEMO.SLACK_SCHEMA.SLACK_PDFS);
...
create or replace table parsed_pdfs as (
    ...
    select
        ...
        'SLACK_SALES_DEMO.SLACK_SCHEMA.SLACK_PDFS' as INPUT_STAGE,
...
create or replace CORTEX SEARCH SERVICE SLACK_SALES_DEMO.SLACK_SCHEMA.info_search
ON PAGE_CONTENT
WAREHOUSE = SLACK_S
TARGET_LAG = '1 hour'
AS (
